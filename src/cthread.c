...
/* Dada a fila do semaforo, imprime o conteúdo da fila (as threads que estão esperando um recurso ser liberado). */
...
/* Verifica se uma thread com um tid passado está em uma PFILA2
   mode é qual o elemento que se deseja comparar.
   Se mode == 0, é comparadp o tid passado com o tid das outras threads (cjoin, precisando procurar na fila de BLOQUEADOS).
   Se mode != 0 compara com awaited_tid, campo criado pelo grupo em cdata.h no TCB_t. Comparação no csignal, precisando ver se alguém esperava a thread liberar o recurso
...
/* Ordena apto de forma crescente por prioridade (tempo de execução)
   Faz isso pegando o elemento que acabou de inserir e percorre APTO até encontrar uma thread com tempo maior de execução (menor prioridade) */
void sortAPTO() {
    ...
    if(LastFila2(&APTO) == 0) {
        ...
        if(FirstFila2(&APTO) == 0) {       // se há alguém na fila além da thread que foi recém inserida, coloca a thread em APTO de acordo com a prioridade
            do {
            ...
            if(notInserted)
                AppendFila2(&APTO, (void *) thread);     // caso onde a thread tem a prioridade mais baixa (tem o maior tempo de execução entre todas)
        }

        else {               // se não havia mais ninguém, então insere ela e acabou
...
void dispatcher() {
    ...
    // exclui de EXECUTANDO thread que terminou sua execução
    if(!cyield_cjoin_cwait_ToDispatcher) {  // se veio para o dispatcher porque terminou sua execução (uc_link = &escalonador)
        if(FirstFila2(&EXECUTANDO) == 0) {        // se há alguma thread executando para encerrar sua execução
            thread = GetAtIteratorFila2(&EXECUTANDO);
...
int cwait(csem_t *sem) {
    ...
    if(sem->count > 0) {       // se recurso está livre
        ...
        if(FirstFila2(&EXECUTANDO) == 0) {        // se há alguma thread executando para colocar em bloqueado
            TCB_t *thread = GetAtIteratorFila2(&EXECUTANDO);
